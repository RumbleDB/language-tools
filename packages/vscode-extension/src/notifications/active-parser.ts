import { ACTIVE_PARSER_NOTIFICATION } from "jsoniq-language-server/notifications";
...
export function registerActiveParserNotification(client: LanguageClient): vscode.Disposable {
    const update = ACTIVE_PARSER_NOTIFICATION.handle(async (payload) => {
    ...
    return client.onNotification(ACTIVE_PARSER_NOTIFICATION.method, update);
}
