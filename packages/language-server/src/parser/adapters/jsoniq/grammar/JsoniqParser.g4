 ...
 * 
 * A parser grammar for JSONiq 1.0/3.1 that includes the XQuery Scripting Extensions, and additional
 * update features. This file is based on the XQuery parser grammar from the xqdoc project:
...
// @header { // Java header package org.rumbledb.parser.jsoniq; }

options {
	tokenVocab = JsoniqLexer;
}

// Mostly taken from http://www.w3.org/TR/xquery/#id-grammar, with some simplifications:
// 
// 1. The parser itself doesn't really enforce ws:explicit except for some easy cases (QNames and
// wildcards). Walkers will need to do this (and also parse wildcards a bit).
// 
// 2. When collecting element content, we will need to check the HIDDEN channel as well, for
// whitespace and XQuery comments (these should be treated as regular text inside elements).
...
module: // replaced with the versionDecl production to match the JSONiq grammar
	(
		KW_JSONIQ KW_VERSION vers = stringLiteral (
...
versionDecl:
	KW_JSONIQ KW_VERSION version = stringLiteral (
		KW_ENCODING encoding = stringLiteral
	)? SEMICOLON;
...
emptyOrderDecl:
	KW_DECLARE KW_DEFAULT KW_ORDER KW_EMPTY emptySequenceOrder = (
		KW_GREATEST
		| KW_LEAST
	);
copyNamespacesDecl:
...
schemaImport:
	KW_IMPORT KW_SCHEMA schemaPrefix? nsURI = uriLiteral (
...
moduleImport:
	KW_IMPORT KW_MODULE (KW_NAMESPACE prefix = ncName EQUAL)? targetNamespace = uriLiteral (
...
contextItemDecl:
	...
	//(KW_AS itemType)? TODO: this is out of spec. However, it is currently kept to match the JSONiq
	// grammar
	(KW_AS sequenceType)?
		// TODO: change to itemType, update expressions to use itemType, update back JSONiq grammar
	(
		(COLON_EQ value = exprSingle)
		| (
			external = KW_EXTERNAL (
				COLON_EQ defaultValue = exprSingle
			)?
		)
	);

// constrains to valid function names only see
// https://www.w3.org/TR/xquery-31/#parse-note-reserved-function-names
functionDecl:
	...
	(
		...
		| is_external = KW_EXTERNAL
	);

// renamed from functionParams to paramList to match the JSONiq grammar
paramList: param (COMMA param)*;

// renamed from functionParam to param to match the JSONiq grammar replaced with the typeDeclaration
// production to match the JSONiq grammar
...
optionDecl:
	KW_DECLARE KW_OPTION name = qname value = stringLiteral;

typeDecl:
	KW_DECLARE KW_TYPE type_name = qname KW_AS (
...
tumblingWindowClause:
	KW_TUMBLING KW_WINDOW DOLLAR name = qname type = typeDeclaration? KW_IN exprSingle
		windowStartCondition windowEndCondition?;

slidingWindowClause:
	KW_SLIDING KW_WINDOW DOLLAR name = qname type = typeDeclaration? KW_IN exprSingle
		windowStartCondition windowEndCondition;
...
orderByClause:
	stb = KW_STABLE? KW_ORDER KW_BY specs += orderByExpr (
		COMMA specs += orderByExpr
	)*;

// renamed from orderSpec to orderByExpr to match the JSONiq grammar
orderByExpr:
	ex = exprSingle (KW_ASCENDING | desc = KW_DESCENDING)? (
		KW_EMPTY (gr = KW_GREATEST | ls = KW_LEAST)
	)? (KW_COLLATION uril = uriLiteral)?;

quantifiedExpr: (so = KW_SOME | ev = KW_EVERY) vars += quantifiedExprVar (
		COMMA vars += quantifiedExprVar
...
switchExpr:
	KW_SWITCH LPAREN cond = expr RPAREN cases += switchCaseClause+ KW_DEFAULT KW_RETURN def =
		exprSingle;

switchCaseClause: (KW_CASE cond += exprSingle)+ KW_RETURN ret = exprSingle;

typeswitchExpr:
	KW_TYPESWITCH LPAREN cond = expr RPAREN cses += caseClause+ KW_DEFAULT (
		var_ref = varRef
	)? KW_RETURN def = exprSingle;

caseClause:
	KW_CASE (var_ref = varRef KW_AS)? union += sequenceType (
		VBAR union += sequenceType
	)* KW_RETURN ret = exprSingle;

ifExpr:
	KW_IF LPAREN test_condition = expr RPAREN KW_THEN branch = exprSingle KW_ELSE else_branch =
		exprSingle;
...
catchClause:
	...
	(
		(jokers += wildcard | errors += eqName) (
			VBAR (jokers += wildcard | errors += eqName)
		)*
		| (LPAREN DOLLAR varName RPAREN)
	)
...
comparisonExpr:
	main_expr = stringConcatExpr (
		op += compOp rhs += stringConcatExpr
	)?;
...
additiveExpr:
	main_expr = multiplicativeExpr (
		op += (PLUS | MINUS) rhs += multiplicativeExpr
	)*;

multiplicativeExpr:
	main_expr = unionExpr (
		op += (STAR | KW_DIV | KW_IDIV | KW_MOD) rhs += unionExpr
	)*;

unionExpr:
	main_expr = intersectExceptExpr (
		op += (KW_UNION | VBAR) rhs += intersectExceptExpr
	)*;

intersectExceptExpr:
	main_expr = instanceOfExpr (
		op += (KW_INTERSECT | KW_EXCEPT) rhs += instanceOfExpr
	)*;

instanceOfExpr:
	main_expr = isStaticallyExpr (
		KW_INSTANCE KW_OF seq = sequenceType
	)?;

isStaticallyExpr:
	main_expr = treatExpr (
		KW_IS KW_STATICALLY seq = sequenceType
	)?;
...
arrowExpr:
	main_expr = unaryExpr (
		ARROW function += arrowFunctionSpecifier arguments += argumentList
	)*;
...
/*
 * this token was added to prevent the antlr error "label assigned to a block which is not a set" in
 * the comparisonExpr token definition
 */
...
// replaced with the enclosedExpression production to match the JSONiq grammar TODO: this is out of
// spec. However, it is currently kept to match the JSONiq grammar TODO: replace with the proper
// rule, throw excep. validateExpr: KW_VALIDATE (validationMode | (KW_TYPE typeName))? LBRACE expr?
// RBRACE ;
validateExpr:
...
pathExpr: (SLASH singleslash = relativePathExpr?)
...
// [96]: we don't check that the closing tag is the same here. It should be done elsewhere, if we
// really want to know. We've also simplified the rule by removing the S? bits from ws:explicit.
// Tree walkers could handle this.
dirElemConstructorOpenClose:
	LANGLE open_tag_name = qname attributes = dirAttributeList endOpen = RANGLE dirElemContent*
		startClose = LANGLE slashClose = SLASH close_tag_name = qname RANGLE;

dirElemConstructorSingleTag:
	LANGLE open_tag_name = qname attributes = dirAttributeList slashClose = SLASH RANGLE;
...
// renamed from inlineFunctionRef to inlineFunctionExpr to match the JSONiq grammar replaced with
// the functionBody production to match the JSONiq grammar
inlineFunctionExpr:
	annotations KW_FUNCTION LPAREN paramList? RPAREN (
		KW_AS return_type = sequenceType
	)? (LBRACE (fn_body = statementsAndOptionalExpr) RBRACE);
...
// TODO: this is out of spec. However, it is currently kept to match the JSONiq grammar singleType:
// item=simpleTypeName (question+=QUESTION)? ; TODO: change to simpletypeName, update expressions.
...
itemType:
	...
	| mapTest
	| arrayTest
	// simplification compared to XQuery 3.1 grammar removes the need for a separate
	// atomicOrUnionType rule
	| KW_NULL
	| eqName
...
attributeTest:
	KW_ATTRIBUTE LPAREN (
		attributeNameOrWildcard (COMMA type = typeName)?
	)? RPAREN;
...
elementTest:
	KW_ELEMENT LPAREN (
		elementNameOrWildcard (
			COMMA type = typeName optional = QUESTION?
		)?
	)? RPAREN;
...
typedFunctionTest:
	KW_FUNCTION LPAREN (
		st += sequenceType (COMMA st += sequenceType)*
	)? RPAREN KW_AS rt = sequenceType;
...
// renamed from qName to qname to match the JSONiq grammar added support for keywords as namespace
// names the FullQName production catches the case where the namespace name is NOT a keyword whereas
// the (ns=ncName COLON)? local_name=ncName production catches the case where the (optional)
// namespace name is a keyword
qname: FullQName | (ns = ncName COLON)? local_name = ncName;

// matches the definition of NCName in the XQuery 3.1 spec this includes all the valid characters,
// including all the keywords
ncName: NCName | keyword;

// function names should be valid NCNames, but limited by the constraint of reserved-function-names
// as defined in the XQuery 3.1 spec see
// https://www.w3.org/TR/xquery-31/#parse-note-reserved-function-names replaced with the FullQName
// production. the FullQName production was removed to prevent ambiguities
functionName:
	FullQName
	| NCName
	| URIQualifiedName
...
// XQuery Scripting Extension ///////////////////////////////////////////////////////////// the
// following section contains rules for the XQuery Scripting Extension Proposal
...
switchStatement:
	KW_SWITCH LPAREN condExpr = expr RPAREN cases += switchCaseStatement+ KW_DEFAULT KW_RETURN def =
		statement;
...
// added to match the JSONiq grammar replaced the CatchErrorList production rule to match the JSONiq
// grammar
catchCaseStatement:
	KW_CATCH (jokers += wildcard | errors += eqName) (
		VBAR (jokers += wildcard | errors += eqName)
	)* catch_block = blockStatement;

// replaced "$" varName with varRef to match the JSONiq grammar
typeSwitchStatement:
	KW_TYPESWITCH LPAREN cond = expr RPAREN cases += caseStatement+ KW_DEFAULT (
		var_ref = varRef
	)? (KW_RETURN) def = statement;

// replaced "$" varName with varRef to match the JSONiq grammar
caseStatement:
	KW_CASE (var_ref = varRef KW_AS)? union += sequenceType (
		VBAR union += sequenceType
	)* (KW_RETURN) ret = statement;

varDeclStatement:
	annotations KW_VARIABLE varDeclForStatement (
		COMMA varDeclForStatement
	)* SEMICOLON;

// added to match the JSONiq grammar
varDeclForStatement:
	var_ref = varRef (KW_AS sequenceType)? (
		COLON_EQ expr_vals += exprSingle
	)?;
...
// Updating expressions (out-of-spec) these are not referenced anywhere in the XQuery spec or in the
// XQuery Scripting Extension spec they are ported from the original grammar, and likely to be
// derived from JSONiq

insertExpr:
	KW_INSERT KW_JSON to_insert_expr = exprSingle KW_INTO main_expr = exprSingle (
		KW_AT KW_POSITION pos_expr = exprSingle
	)?
...
transformExpr:
	KW_COPY copyDecl (COMMA copyDecl)* KW_MODIFY mod_expr = exprSingle KW_RETURN ret_expr =
		exprSingle;
...
createCollectionExpr:
	KW_CREATE KW_COLLECTION collectionMode = (
		KW_TABLE
		| KW_DELTA_FILE
		| KW_ICEBERG_TABLE
	) LPAREN collection_name = exprSimple RPAREN (
		KW_WITH content = exprSingle
	)?;

deleteIndexExpr:
	KW_DELETE (
		(first = KW_FIRST | last = KW_LAST) num = exprSingle?
	) KW_FROM KW_COLLECTION collectionMode = (
		KW_TABLE
		| KW_DELTA_FILE
		| KW_ICEBERG_TABLE
	) LPAREN collection_name = exprSimple RPAREN;
...
insertIndexExpr:
	KW_INSERT content = exprSingle (
		(KW_AT pos = exprSingle)
		| first = KW_FIRST
		| last = KW_LAST
	) KW_INTO KW_COLLECTION collectionMode = (
		KW_TABLE
		| KW_DELTA_FILE
		| KW_ICEBERG_TABLE
	) LPAREN collection_name = exprSimple RPAREN;
...
insertSearchExpr:
	KW_INSERT content = exprSingle (
		before = KW_BEFORE
		| after = KW_AFTER
	) target = exprSingle KW_INTO KW_COLLECTION;

truncateCollectionExpr: (KW_DELETE | KW_TRUNCATE) KW_COLLECTION collectionMode = (
		KW_TABLE
		| KW_DELTA_FILE
		| KW_ICEBERG_TABLE
	) LPAREN collection_name = exprSimple RPAREN;
